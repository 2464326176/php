...
class AjaxUpload
{
    ...
    public function __construct($form_name, $file_size) {
        ...
        $this->upload_dir = ROOT_PATH . 'public' . DS . 'uploads/testpaper/';
    ...
    public function upload($return_arr=false) {
        ...
        if (!move_uploaded_file($file['tmp_name'], $filename)) { //执行上传
            ...
            $data = [
                'picurl' => '/lyzg/public/uploads/testpaper/'.$fname,
            ];
            $datas=db('content')->insert($data);
            $arr = array('ok'=>1, 'filename'=>'/lyzg/public/uploads/testpaper/'.$fname, 'size'=>$file['size']);
            if ($return_arr)
                return $arr;
            else {

                echo json_encode(array('ok'=>$arr['ok'], 'filename'=>'/lyzg/public/uploads/testpaper/'.$fname)); exit;
            }
        }
    }
