...
Page({
  ...
  leaveSubmit: function () {
    ...
    wx.request({
      url: 'https://liuyuhang.xin/lyzg/public/index.php/wxapi/plan/leave',
      data: {
        taskID: d.curTask.num,
        openId: openId,
        reason: d.reason
      },
      method: 'POST',
  ...
  */
  updateTask: function (openId) {
    ...
    wx.request({
      url: 'https://liuyuhang.xin/lyzg/public/index.php/wxapi/plan/plan_lists',
      data: {
        'openId': openId
      },
      method: 'GET', 
      success: function(res){
        ...
        var tasks = res.data.sort(function (a, b) {
        ...
        for (var j = 0, len = tasks.length; j < len; ++j) {
          ...
          wx.request({
            url: 'https://liuyuhang.xin/lyzg/public/index.php/wxapi/plan/get_planum',
            data:{
              openid: tasks[j].openid,
              num: tasks[j].num,
              code:j,
            },
            method: 'GET', 
            success:function(planum_data){ 
              if (planum_data.data.data=="no"){
               
                var signstate = {
                  "signstate": 0
                }
                var json_tasks = that.mergeJsonObject(signstate, tasks[planum_data.data.code]);
                tmpTasks.push(json_tasks);

                
              }else{   
                var signstate={
                  "signstate": planum_data.data.data.signstate
                }
                var json_tasks = that.mergeJsonObject(signstate, tasks[planum_data.data.code]);
                tmpTasks.push(json_tasks);
              }             
        ...
        wx.getLocation({
          ...
          success: function(res){
            ...
            that.setData({
              ...
              currentSignstate: tmpTasks[that.data.current].signstate,

              curTask: tmpTasks[that.data.current],
              currentLeaveStart: tmpTasks[that.data.current].startDay,
              currentLeaveEnd: tmpTasks[that.data.current].endDay,
  ...
  changeSwiper: function (e) {
    ...
    this.setData({
      ...
      currentSignstate: this.data.tasks[e.detail.current].signstate,
      curTask: this.data.tasks[e.detail.current],
      currentLeaveStart: this.data.tasks[e.detail.current].startDay,
      currentLeaveEnd: this.data.tasks[e.detail.current].endDay
    })
        ...
        success: function(res){
          ...
          if (d < 2) {
            ...
            if (that.data.currentTimeFlag == 0 && parseInt(that.data.clock.hour) > 2) {
              ...
              wx.request({
                url: 'https://liuyuhang.xin/lyzg/public/index.php/wxapi/plan/punch?time='+ (new Date()).getTime(),
                data: {
                  openId: openId,
                  currentTimeFlag:that.data.currentTimeFlag,
                  taskID: curTask.num,
